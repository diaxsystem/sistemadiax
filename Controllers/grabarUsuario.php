...
if (!empty($_POST)) {
    if (empty($_POST['nombre']) || empty($_POST['correo']) || empty($_POST['usuario']) || empty($_POST['pass']) || empty($_POST['rol'])) {
        ...
        $query = mysqli_query($conection,"SELECT * FROM usuario WHERE cedula = '$cedula' or usuario = '$usuario' or correo = '$correo'");
        ...
        if ($resultado > 0) {
            ...
            $query_insert = mysqli_query($conection,"INSERT INTO usuario(cedula,nombre,correo,usuario,pass,rol)
